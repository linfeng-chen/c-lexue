...
int main(){
    ...
    if(zhuxiqu.size()!=0&&zhuhequ.size()!=0) {
        for (int i = 0; i < zhuxiqu.size(); ++i) {
            printf("m%d", zhuxiqu[i]);
            if (i != zhuxiqu.size() - 1)printf(" ∨ ");
        }
        printf(" ; ");
        for (int i = 0; i < zhuhequ.size(); ++i) {
            printf("M%d", zhuhequ[i]);
            if (i != zhuhequ.size() - 1)printf(" ∧ ");
        }
        printf("\n");
    } else
    {
        if (zhuxiqu.size()==1<<count){
            for (int i = 0; i < zhuxiqu.size(); ++i) {
                printf("m%d", zhuxiqu[i]);
                if (i != zhuxiqu.size() - 1)printf(" ∨ ");
            }
